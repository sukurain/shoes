...
## 可复制示例 (Runnable Examples)

以下示例可以作为完整配置文件使用，运行前替换证书、密钥、密码和上游地址：

- `examples/hysteria2_server_bbr.yaml` - Hysteria2 服务端，QUIC BBR
- `examples/tuic_v5_bbr.yaml` - TUIC v5 服务端，QUIC BBR
- `examples/quic_client_bbr.yaml` - 本地 SOCKS5，经 QUIC+BBR 上游出站
- `examples/wireguard_outbound_basic.yaml` - 本地 SOCKS5，经标准 WireGuard peer 出站
...
quic_settings:
  ...
  congestion: default | bbr    # 可选，默认: default
...
protocol:
  type: wireguard
  private-key: string          # 本机 WireGuard 私钥 (base64)
  server: string               # 对端服务器地址或主机名
  port: 51820
  ip: 10.8.0.2                 # 本机隧道 IPv4
  ipv6: 2001:db8:100::2        # 可选的本机隧道 IPv6
  public-key: string           # 对端公钥 (base64)
  pre-shared-key: string       # 可选的预共享密钥 (base64)
  allowed-ips: ["0.0.0.0/0", "::/0"]
  udp: true                    # 默认: true
  mtu: 1420                    # 默认: 1420
  ip-version: ipv4-prefer      # ipv4-only | ipv6-only | ipv4-prefer | ipv6-prefer
  remote-dns-resolve: false    # 通过 WireGuard 内的 DNS 解析目标主机名
  dns: ["1.1.1.1"]             # remote-dns-resolve=true 时必填
```

WireGuard 是 socket-level outbound，只能作为 client chain 的第一个 hop 使用，不能嵌套在 TLS、Reality、ShadowTLS 或 WebSocket 内。标准 WireGuard 不支持 WARP `reserved`/client id 扩展；配置里出现 `reserved` 会被视为未知字段。
...
- 需要尝试 BBR 时显式设置 `quic_settings.congestion: bbr`；未设置时使用 Quinn 默认拥塞控制
