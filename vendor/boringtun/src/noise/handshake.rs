...
impl Handshake {
    ...
    pub(super) fn format_handshake_initiation<'a>(
        ...
        message_type[0] = super::HANDSHAKE_INIT as u8;
        message_type[1..4].copy_from_slice(&self.reserved);
        // msg.sender_index = little_endian(initiator.sender_index)
    ...
    fn format_handshake_response<'a>(
        ...
        let ephemeral_private = x25519::ReusableSecret::random_from_rng(OsRng);
        let local_index = self.inc_index();
        message_type[0] = super::HANDSHAKE_RESP as u8;
        message_type[1..4].copy_from_slice(&self.reserved);
        ...
        Ok((
            dst,
            Session::new(local_index, peer_index, temp2, temp3, self.reserved),
        ))
    }
}
